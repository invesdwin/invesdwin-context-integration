...
@ThreadSafe
public final class NoMpiJobMainJar extends MergedClasspathJar {

    private final Log log = new Log(this);

    public NoMpiJobMainJar(final IMergedClasspathJarFilter filter) {
        super(filter);
    }
    ...
    protected JarOutputStream newJarOutputStream(final FileOutputStream fos) throws IOException {
        ...
        global.put(Attributes.Name.MAIN_CLASS, NoMpiJobMain.class.getName());
        return new JarOutputStream(fos, manifest);
    }

    @Override
    protected File newFile() {
        return new File(ContextProperties.getCacheDirectory(), NoMpiJobMain.class.getSimpleName() + ".jar");
    }
