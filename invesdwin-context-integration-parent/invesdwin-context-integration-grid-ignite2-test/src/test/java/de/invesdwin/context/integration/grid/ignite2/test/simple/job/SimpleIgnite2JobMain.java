...
@NotThreadSafe
public class SimpleIgnite2JobMain extends AMain {
    ...
    public SimpleIgnite2JobMain() {
        super(Strings.EMPTY_ARRAY, BOOTSTRAP);
    }

    public SimpleIgnite2JobMain(final String[] args) {
    ...
    public static void runIgniteJob(final String logDir, final int size, final String master) {
        ...
        try (Ignite ignite = Ignition.start(cfg)) {
            final List<SimpleIgnite2Task> tasks = new ArrayList<>();
            for (int rank = 0; rank < size; rank++) {
                tasks.add(new SimpleIgnite2Task(rank, size));
            }

            // Distribute and execute worker tasks across the compute grid
            final Collection<SimpleIgnite2Task.TaskResult> results = ignite.compute().call(tasks);
    ...
    public static void main(final String[] args) {
        new SimpleIgnite2JobMain(args).run();
    }
}