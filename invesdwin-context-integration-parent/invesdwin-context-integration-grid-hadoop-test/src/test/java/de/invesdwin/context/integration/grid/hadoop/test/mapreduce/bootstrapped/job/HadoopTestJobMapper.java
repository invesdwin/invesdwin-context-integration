...
import de.invesdwin.context.integration.grid.hadoop.mapreduce.AMapper;
...
public class HadoopTestJobMapper extends AMapper<LongWritable, Text, Text, Text> {
    ...
    private static final Log LOG = new Log(HadoopTestJobMapper.class);
    ...
    private void checkDuplicateClasses() {
        new ClasspathResourceProcessor().process(new IClasspathResourceVisitor() {
            ...
            public void finish() {
                ...
                if (duplicates > 0) {
                    LOG.warn("Found %s duplicate classes introduced by job: %s", duplicates, sb);
                }
            }
        });
    }
    ...
    private void checkGetResource() throws IOException {
        final Resource resource = PreMergedContext.getInstance()
                .getResource("classpath:/" + HadoopTestJobMapperIsolatedScannedBean.class.getName().replace(".", "/")
                        + ".class");
        Assertions.assertThat(resource.contentLength()).isGreaterThan(0);

        final Resource[] resourcesAsterisk = PreMergedContext.getInstance()
                .getResources("classpath*:/" + HadoopTestJobMapperIsolatedScannedBean.class.getName().replace(".", "/")
                        + ".class");
    ...
    private void checkClasspathScanning() {
        ...
        final Set<BeanDefinition> findCandidateComponents = provider
                .findCandidateComponents(HadoopTestJobMapperIsolatedScannedBean.class.getPackage().getName());
    ...
    private void checkIsolatedSpringContext() {
        ...
        Assertions.assertThat(Reflections.classExists(HadoopTestJobMapperIsolatedScannedBean.class.getName()))
                .isNotNull();
        try {
            Assertions.assertThat(Class.forName(HadoopTestJobMapperIsolatedScannedBean.class.getName(), true,
                    ClassUtils.getDefaultClassLoader())).isNotNull();
        } catch (final ClassNotFoundException e) {
            throw Err.process(e);
        }

        final HadoopTestJobMapperIsolatedScannedBean scannedBean = ctx
                .getBean(HadoopTestJobMapperIsolatedScannedBean.class);
    ...
    @Configurable
    private static final class LoadTimeWeavingTest {
