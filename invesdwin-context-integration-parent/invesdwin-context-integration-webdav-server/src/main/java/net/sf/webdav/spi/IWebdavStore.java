...
public interface IWebdavStore {
    ...
    /**
     * Indicates that a new request or transaction with this store involved has been started. The request will be
     * terminated by either {@link #commit(ITransaction transaction)} or {@link #rollback(ITransaction transaction)}. If
     * only non-read methods have been called, the request will be terminated by a
     * {@link #commit(ITransaction transaction)}. This method will be called by (@link WebdavStoreAdapter} at the
     * beginning of each request.
     * 
     * 
     * @param principal
     *            the principal that started this request or <code>null</code> if there is non available
     * 
     ...
     */
    void checkAuthentication(ITransaction transaction) throws WebdavException;

    /**
     * Indicates that all changes done inside this request shall be made permanent and any transactions, connections and
     * other temporary resources shall be terminated.
     * 
     * @param transaction
     *            indicates that the method is within the scope of a WebDAV transaction
     * 
    ...
    /**
     * Indicates that all changes done inside this request shall be undone and any transactions, connections and other
     ...
     * @param resourceUri
     *            URI of the content resource
     * @return length of the resource in bytes, <code>-1</code> declares this value as invalid and asks the adapter to
     *         try to set it from the properties if possible
     * @throws WebdavException
     *             if something goes wrong on the store level
     */
     ...
     * @return StoredObject
     */
    StoredObject getStoredObject(ITransaction transaction, String uri) throws WebdavException;
