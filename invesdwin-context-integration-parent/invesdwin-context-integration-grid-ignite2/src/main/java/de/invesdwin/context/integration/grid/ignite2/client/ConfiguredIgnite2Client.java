...
public final class ConfiguredIgnite2Client implements FactoryBean<IgniteClient> {
    ...
    private static void waitForWarmup() {
        final int minimumServersCount = 1;
        final int minimumNodesCount = 0;
        try {
            processingThreadsCounter.waitForMinimumCounts(minimumNodesCount, minimumServersCount, Duration.ONE_MINUTE);
