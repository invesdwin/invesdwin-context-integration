...
public class PurgeOldTempFilesScheduler implements IStartupHook {
    ...
    public void purgeOldFiles() {
        ...
        final FDate threshold = new FDate().subtract(WebdavClientProperties.PURGE_TEMP_FILES_OLDER_THAN_DURATION);
        final Iterator<File> filesToDelete = FileUtils.iterateFiles(WebdavClientProperties.TEMP_DIRECTORY,
                new AgeFileFilter(threshold.dateValue(), true), TrueFileFilter.INSTANCE);
        while (filesToDelete.hasNext()) {
            final File fileToDelete = filesToDelete.next();
            fileToDelete.delete();
        }
        for (final File f : WebdavClientProperties.TEMP_DIRECTORY.listFiles()) {
