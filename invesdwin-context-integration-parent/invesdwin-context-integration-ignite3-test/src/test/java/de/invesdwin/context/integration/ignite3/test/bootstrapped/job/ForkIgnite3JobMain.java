...
public class ForkIgnite3JobMain extends AMain {
    ...
    protected void startApplication(final CmdLineParser parser) {
        try {
            // 1. Deploy the Job JAR directly via Ignite 3 REST API (Multipart POST)
            final String unitId = "fork-job-unit";
            final String unitVersion = "3.0.0";
            deployUnitViaRest(restAddress, unitId, unitVersion, jobJar);
            ...
            try (IgniteClient client = IgniteClient.builder().addresses(master).build()) {
                ...
                // 4. Submit compute job for each rank
                for (int rank = 0; rank < size; rank++) {
                    // Pass parameters as a delimited string so the worker node knows its rank
                    final String args = rank + ";" + size + ";" + jobJar + ";" + master;

                    // submitAsync returns CompletableFuture<JobExecution<R>>.
                    // We use thenCompose to unwrap it into CompletableFuture<R>.
                ...
                // 5. Gather results and write logs to host
    ...
    private void deployUnitViaRest(final String restAddress, final String unitId, final String version,
        ...
        // Changed name=\"unit\" to name=\"unitContent\" to satisfy the Ignite 3 REST controller
