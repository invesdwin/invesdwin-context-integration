...
@NotThreadSafe
public class BootstrappedIgnite3ServerJobMain extends AMain {
    ...
    public BootstrappedIgnite3ServerJobMain(final String[] args) {
    ...
    protected void startApplication(final CmdLineParser parser) {
        ...
        try {
            ...
            if (master) {
                ...
                final JobDescriptor.Builder<String, BootstrappedIgnite3ServerTask.TaskResult> descriptorBuilder = JobDescriptor.<String, BootstrappedIgnite3ServerTask.TaskResult> builder(
                        BootstrappedIgnite3ServerTask.class.getName())
                        .resultClass(BootstrappedIgnite3ServerTask.TaskResult.class);

                if (jobJar != null) {
                    descriptorBuilder.units(List.of(new DeploymentUnit(unitId, unitVersion)));
                }

                final JobDescriptor<String, BootstrappedIgnite3ServerTask.TaskResult> descriptor = descriptorBuilder
                        .build();

                final JobTarget target = JobTarget.anyNode(ignite.cluster().nodes());
                final List<CompletableFuture<BootstrappedIgnite3ServerTask.TaskResult>> futures = new ArrayList<>();

                for (int rank = 0; rank < size; rank++) {
                    final String args = rank + ";" + size;
                    final CompletableFuture<BootstrappedIgnite3ServerTask.TaskResult> future = ignite.compute()
    ...
    public static void main(final String[] args) {
        new BootstrappedIgnite3ServerJobMain(args).run();
    }
}