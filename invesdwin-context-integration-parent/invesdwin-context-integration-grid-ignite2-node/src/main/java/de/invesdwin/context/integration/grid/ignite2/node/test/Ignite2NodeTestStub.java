...
import de.invesdwin.context.integration.grid.ignite2.node.ConfiguredIgnite2Node;
...
@Named
@ThreadSafe
public class Ignite2NodeTestStub extends StubSupport {

    @GuardedBy("this.class")
    private static ConfiguredIgnite2Node lastNode;
    ...
    public void setUpContextLocations(final ATest test, final List<PositionedResource> locations) throws Exception {
        final Ignite2NodeTest annotation = Reflections.getAnnotation(test, Ignite2NodeTest.class);
        if (annotation != null) {
            if (annotation.value()) {
                locations.add(Ignite2NodeContextLocation.NODE_CONTEXT_LOCATION);
            } else {
                locations.remove(Ignite2NodeContextLocation.NODE_CONTEXT_LOCATION);
            }
        }
    }
    ...
    public void setUpOnce(final ATest test, final ITestContext ctx) throws Exception {
        synchronized (Ignite2NodeTestStub.class) {
            if (Ignite2NodeTestStub.lastNode == null) {
                try {
                    Ignite2NodeTestStub.lastNode = MergedContext.getInstance().getBean(ConfiguredIgnite2Node.class);
