...
public class Ignite2NodeContextLocation extends AConditionalContextLocation {
    ...
    public static final PositionedResource NODE_PUBLICATION_CONTEXT_LOCATION;
    ...
    static {
        ...
        NODE_PUBLICATION_CONTEXT_LOCATION = PositionedResource
                .of(new ClassPathResource("/META-INF/ctx.ignite2.server.xml"), ResourcePosition.START);
    }

    @Override
    protected List<PositionedResource> getContextResourcesIfConditionSatisfied() {
        if (!Ignite2NodeProperties.CLIENT_MODE) {
            return Arrays.asList(NODE_CONTEXT_LOCATION, NODE_PUBLICATION_CONTEXT_LOCATION);
        } else {
