...
/**
 * RMI exporter that exposes the specified service as RMI object with the specified name. Such services can be accessed
 * via plain RMI or via {@link RmiProxyFactoryBean}. Also supports exposing any non-RMI service via RMI invokers, to be
 * accessed via {@link RmiClientInterceptor} / {@link RmiProxyFactoryBean}'s automatic detection of such invokers.
 *
 * <p>
 * With an RMI invoker, RMI communication works on the {@link RmiInvocationHandler} level, needing only one stub for any
 * service. Service interfaces do not have to extend {@code java.rmi.Remote} or throw {@code java.rmi.RemoteException}
 ...
 *
 * <p>
 * Note: RMI makes a best-effort attempt to obtain the fully qualified host name. If one cannot be determined, it will
 * fall back and use the IP address. Depending on your network configuration, in some cases it will resolve the IP to
 * the loopback address. To ensure that RMI will use the host name bound to the correct network interface, you should
 * pass the {@code java.rmi.server.hostname} property to the JVM that will export the registry and/or the service using
 * the "-D" JVM argument. For example: {@code -Djava.rmi.server.hostname=myserver.com}
 *
...
public class RmiServiceExporter extends RmiBasedExporter implements InitializingBean, DisposableBean {
     ...
     * <p>
     * If the given object also implements {@code java.rmi.server.RMIServerSocketFactory}, it will automatically be
     * registered as server socket factory too.
     *
     ...
     * <p>
     * Only needs to be specified when the client socket factory does not implement
     * {@code java.rmi.server.RMIServerSocketFactory} already.
     *
    ...
    /**
     * Specify the RMI registry to register the exported service with. Typically used in combination with
     * RmiRegistryFactoryBean.
     * <p>
     * Alternatively, you can specify all registry properties locally. This exporter will then try to locate the
     * specified registry, automatically creating a new local one if appropriate.
     * <p>
    ...
    /**
     * Set whether to always create the registry in-process, not attempting to locate an existing registry at the
     * specified port.
     * <p>
     * Default is "false". Switch this flag to "true" in order to avoid the overhead of locating an existing registry
     * when you always intend to create a new registry in any case.
     */
    ...
    /**
     * Set whether to replace an existing binding in the RMI registry, that is, whether to simply override an existing
     * binding with the specified service in case of a naming conflict in the registry.
     * <p>
     * Default is "true", assuming that an existing binding for this exporter's service name is an accidental leftover
     * from a previous execution. Switch this to "false" to make the exporter fail in such a scenario, indicating that
     * there was already an RMI object bound.
     */
    ...
    public void prepare() throws RemoteException {
        ...
        if (this.registry == null) {
            this.registry = getRegistry(this.registryHost, this.registryPort, this.registryClientSocketFactory,
     ...
     *
     * @param registryHost
     *            the registry host to use (if this is specified, no implicit creation of a RMI registry will happen)
     * @param registryPort
     *            the registry port to use
     * @param clientSocketFactory
     *            the RMI client socket factory for the registry (if any)
     ...
     */
    protected Registry getRegistry(final String registryHost, final int registryPort,
            @Nullable final RMIClientSocketFactory clientSocketFactory,
            @Nullable final RMIServerSocketFactory serverSocketFactory) throws RemoteException {
     ...
     *             if the registry couldn't be located or created
     */
    protected Registry getRegistry(final int registryPort, @Nullable final RMIClientSocketFactory clientSocketFactory,
            @Nullable final RMIServerSocketFactory serverSocketFactory) throws RemoteException {
