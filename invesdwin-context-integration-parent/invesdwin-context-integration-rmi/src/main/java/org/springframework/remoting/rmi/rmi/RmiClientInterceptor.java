...
import org.springframework.remoting.rmi.RemoteConnectFailureException;
import org.springframework.remoting.rmi.RemoteInvocationFailureException;
import org.springframework.remoting.rmi.RemoteLookupFailureException;
import org.springframework.remoting.rmi.support.RemoteInvocationBasedAccessor;
import org.springframework.remoting.rmi.support.RemoteInvocationUtils;

/**
 * {@link org.aopalliance.intercept.MethodInterceptor} for accessing conventional RMI services or RMI invokers. The
 * service URL must be a valid RMI URL (e.g. "rmi://localhost:1099/myservice").
 *
 * <p>
 * RMI invokers work at the RmiInvocationHandler level, needing only one stub for any service. Service interfaces do not
 * have to extend {@code java.rmi.Remote} or throw {@code java.rmi.RemoteException}. Spring's unchecked
 * RemoteAccessException will be thrown on remote invocation failure. Of course, in and out parameters have to be
 * serializable.
 *
 * <p>
 * With conventional RMI services, this invoker is typically used with the RMI service interface. Alternatively, this
 * invoker can also proxy a remote RMI service with a matching non-RMI business interface, i.e. an interface that
 ...
 * @see org.springframework.remoting.rmi.RemoteAccessException
...
public class RmiClientInterceptor extends RemoteInvocationBasedAccessor implements MethodInterceptor {
     ...
     * <p>
     * Can be turned off to allow for late start of the RMI server. In this case, the RMI stub will be fetched on first
     * access.
     *
     * @see #setCacheStub
     */
     ...
     * <p>
     * Can be turned off to allow for hot restart of the RMI server. In this case, the RMI stub will be fetched for each
     * invocation.
     *
     ...
     * <p>
     * Can be turned on to allow for hot restart of the RMI server. If a cached RMI stub throws an RMI exception that
     * indicates a remote connect failure, a fresh proxy will be fetched and the invocation will be retried.
     *
    ...
    public void prepare() throws RemoteLookupFailureException {
        ...
        if (this.lookupStubOnStartup) {
            final Remote remoteObj = lookupStub();
            if (logger.isDebugEnabled()) {
                if (remoteObj instanceof RmiInvocationHandler) {
                    logger.debug("RMI stub [" + getServiceUrl() + "] is an RMI invoker");
                } else if (getServiceInterface() != null) {
                    final boolean isImpl = getServiceInterface().isInstance(remoteObj);
     ...
     * <p>
     * Called on interceptor initialization if "cacheStub" is "true"; else called for each invocation by
     * {@link #getStub()}.
     * <p>
     * The default implementation looks up the service URL via {@code java.rmi.Naming}. This can be overridden in
     * subclasses.
     *
    ...
    protected Remote lookupStub() throws RemoteLookupFailureException {
        try {
            ...
            if (this.registryClientSocketFactory != null) {
                ...
                final URL url = new URL(null, getServiceUrl(), new DummyURLStreamHandler());
     ...
     * <p>
     * The default implementation returns the stub created on initialization, if any. Else, it invokes
     * {@link #lookupStub} to get a new stub for each invocation. This can be overridden in subclasses, for example in
     * order to cache a stub for a given amount of time before recreating it, or to test the stub whether it is still
     * alive.
     *
    ...
    /**
     * Fetches an RMI stub and delegates to {@code doInvoke}. If configured to refresh on connect failure, it will call
     ...
     * <p>
     * The default implementation delegates to {@link RmiClientInterceptorUtils#isConnectFailure}.
     *
     * @param ex
     ...
     * @throws Throwable
     *             an exception raised by the new invocation, if it failed as well
    ...
    private Object handleRemoteConnectFailure(final MethodInvocation invocation, final Exception ex) throws Throwable {
        if (this.refreshStubOnConnectFailure) {
            final String msg = "Could not connect to RMI service [" + getServiceUrl() + "] - retrying";
            if (logger.isDebugEnabled()) {
                logger.warn(msg, ex);
    ...
    protected Object doInvoke(final MethodInvocation invocation, final Remote stub) throws Throwable {
        if (stub instanceof RmiInvocationHandler) {
            ...
            try {
                ...
                final Throwable exToThrow = ex.getTargetException();
     ...
     * @see org.springframework.remoting.rmi.support.RemoteInvocation
     */
    @Nullable
    protected Object doInvoke(final MethodInvocation methodInvocation, final RmiInvocationHandler invocationHandler)
    ...
    /**
     * Dummy URLStreamHandler that's just specified to suppress the standard {@code java.net.URL} URLStreamHandler
     * lookup, to be able to use the standard URL class for parsing "rmi:..." URLs.
     */
