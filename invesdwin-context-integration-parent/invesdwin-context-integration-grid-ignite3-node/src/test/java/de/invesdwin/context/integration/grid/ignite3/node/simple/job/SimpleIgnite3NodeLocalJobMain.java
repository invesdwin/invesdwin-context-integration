...
@NotThreadSafe
public class SimpleIgnite3NodeLocalJobMain extends AMain {
        ...
        try {
            ...
            if (master) {
                ...
                for (int rank = 0; rank < size; rank++) {
                    ...
                    final CompletableFuture<SimpleIgnite3NodeTask.TaskResult> future = ignite.compute()
                            .submitAsync(target, descriptor, args)
                            .thenCompose(org.apache.ignite.compute.JobExecution::resultAsync);
                    futures.add(future);
                }

                for (final CompletableFuture<SimpleIgnite3NodeTask.TaskResult> future : futures) {
                    final SimpleIgnite3NodeTask.TaskResult result = future.join();
    ...
    public static void main(final String[] args) {
        new SimpleIgnite3NodeLocalJobMain(args).run();
    }
}