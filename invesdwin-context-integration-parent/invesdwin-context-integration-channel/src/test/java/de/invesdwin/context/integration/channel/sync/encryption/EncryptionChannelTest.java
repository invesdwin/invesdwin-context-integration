...
public class EncryptionChannelTest extends AChannelTest {
    ...
    static {
        try (CryptoRandomGenerator random = CryptoRandomGenerators.newSecureRandom()) {
            final byte[] key = AesKeyLength._256.newRandomBytes(random);
            final byte[] iv = AesKeyLength._128.newRandomBytes(random);
            CRYPTO_FACTORY = new AesEncryptionFactory(AesAlgorithm.DEFAULT, key, iv);
        }
    }

    @Test
    public void testCryptoPerformance() throws InterruptedException {
