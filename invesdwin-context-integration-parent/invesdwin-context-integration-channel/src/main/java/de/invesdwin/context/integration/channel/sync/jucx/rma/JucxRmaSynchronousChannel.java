...
public class JucxRmaSynchronousChannel implements IJucxSynchronousChannel {
    ...
    public JucxRmaSynchronousChannel(final InetSocketAddress socketAddress, final boolean server,
    ...
    void establishConnection() throws IOException {
        ...
        final UcpRequest sendRequest = finalizer.ucpEndpoint.sendTaggedNonBlocking(buffer,
                errorUcxCallback.maybeThrowAndReset());
        requestSpinWait.waitForRequest(sendRequest, getConnectTimeout());
        final UcpRequest recvRequest = finalizer.ucpWorker.recvTaggedNonBlocking(buffer,
                errorUcxCallback.maybeThrowAndReset());
    ...
    private static final class UcxSynchronousChannelFinalizer extends AFinalizer {
        ...
        protected void onRun() {
            String warning = "Finalizing unclosed " + JucxRmaSynchronousChannel.class.getSimpleName();
            if (Throwables.isDebugStackTraceEnabled()) {
