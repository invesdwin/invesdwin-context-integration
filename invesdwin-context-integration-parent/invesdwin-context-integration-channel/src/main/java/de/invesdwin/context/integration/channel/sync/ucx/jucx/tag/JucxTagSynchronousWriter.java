...
@NotThreadSafe
public class JucxTagSynchronousWriter implements ISynchronousWriter<IByteBufferProvider> {

    private JucxTagSynchronousChannel channel;
    ...
    public JucxTagSynchronousWriter(final JucxTagSynchronousChannel channel) {
    ...
    private boolean writeFurther() throws IOException {
        if (request == null) {
            request = channel.getUcpEndpoint()
                    .sendTaggedNonBlocking(buffer.addressOffset() + position, remaining, channel.getLocalTag(),
