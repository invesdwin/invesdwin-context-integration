...
public class TomcatNativeSocketSynchronousChannel implements Closeable {
    ...
    public void open() throws IOException {
        ...
        if (server) {
            awaitSession(() -> {
                try {
                    new AprServerOpener2().openServer(TomcatNativeSocketSynchronousChannel.this);
            ...
            awaitSession(() -> {
                try {
                    new AprClientOpener2().openClient(TomcatNativeSocketSynchronousChannel.this);
                } catch (final Exception e) {
                    throw new RuntimeException(e);
                }
            });
        ...
        final int rv = Poll.add(finalizer.pollset, finalizer.getFd(), Poll.APR_POLLIN);
    ...
    private void init() {
        ...
        long newPollset;
        try {
            newPollset = Poll.create(AprServerOpener.POLLSET_SIZE, finalizer.pool, Poll.APR_POLLSET_THREADSAFE,
                    Long.MAX_VALUE);

            if (newPollset == 0) {
                newPollset = Poll.create(62, finalizer.pool, Poll.APR_POLLSET_THREADSAFE, Long.MAX_VALUE);
            }

            finalizer.pollset = newPollset;
            if (finalizer.pollset < 0) {
                if (Status.APR_STATUS_IS_ENOTIMPL(-(int) finalizer.pollset)) {
    ...
    NettySocketSynchronousChannelFinalizer getFinalizer() {
        return finalizer;
    }

    public static final class NettySocketSynchronousChannelFinalizer extends AFinalizer {
        ...
        public void setPollset(final long pollset) {
