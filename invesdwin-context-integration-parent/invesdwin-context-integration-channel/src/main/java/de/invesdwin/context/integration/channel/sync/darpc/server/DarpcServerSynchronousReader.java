...
public class DarpcServerSynchronousReader implements ISynchronousReader<IByteBufferProvider> {
    ...
    public IByteBufferProvider readMessage() throws IOException {
        ...
        if (ClosedByteBuffer.isClosed(message.getMessage(), DarpcServerSynchronousChannel.MESSAGE_INDEX, size)) {
            close();
            throw FastEOFException.getInstance("closed by other side");
        }
        return message.getMessage().slice(DarpcServerSynchronousChannel.MESSAGE_INDEX, size);
    }
