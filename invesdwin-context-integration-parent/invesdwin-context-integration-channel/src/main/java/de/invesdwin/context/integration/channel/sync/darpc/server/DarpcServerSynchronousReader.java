...
@NotThreadSafe
public class DarpcServerSynchronousReader implements ISynchronousReader<IByteBufferProvider> {
    ...
    public DarpcServerSynchronousReader(final DarpcServerSynchronousChannel channel) {
    ...
    private boolean hasMessage() throws IOException {
        if (messageTargetPosition == 0) {
            final int sizeTargetPosition = bufferOffset + DarpcServerSynchronousChannel.MESSAGE_INDEX;
            ...
            final int size = buffer.getInt(bufferOffset + DarpcServerSynchronousChannel.SIZE_INDEX);
            if (size <= 0) {
                close();
    ...
    private boolean readFurther(final int targetPosition, final int readLength) throws IOException {
        if (nioBuffer.position() < targetPosition) {
            if (channel.getEndpoint().isRecvFinished()) {
                ...
                ByteBuffers.position(nioBuffer, bufferOffset + DarpcServerSynchronousChannel.MESSAGE_INDEX + size);
            }
        }
    ...
    public IByteBufferProvider readMessage() throws IOException {
        final int size = messageTargetPosition - bufferOffset - DarpcServerSynchronousChannel.MESSAGE_INDEX;
        if (ClosedByteBuffer.isClosed(buffer, bufferOffset + DarpcServerSynchronousChannel.MESSAGE_INDEX, size)) {
            close();
            throw FastEOFException.getInstance("closed by other side");
        }

        final IByteBuffer message = buffer.slice(bufferOffset + DarpcServerSynchronousChannel.MESSAGE_INDEX, size);
        final int offset = DarpcServerSynchronousChannel.MESSAGE_INDEX + size;
