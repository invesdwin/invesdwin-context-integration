...
@NotThreadSafe
public class JucxRmaSynchronousReader implements ISynchronousReader<IByteBufferProvider> {

    private JucxRmaSynchronousChannel channel;
    ...
    public JucxRmaSynchronousReader(final JucxRmaSynchronousChannel channel) {
        ...
        if (position < targetPosition) {
            if (request == null) {
                request = channel.getUcpEndpoint()
                        .getNonBlocking(channel.getRemoteAddress(), channel.getUcpRemoteKey(),
                                buffer.addressOffset() + position, readLength, channel.getErrorUcxCallback().reset());
            }
            try {
