...
Network    NettyDatagramNio (loopback)            Records:     48.28/ms    => ~55% slower
...
Network    BidiNettySocketEpoll (loopback)        Records:    114.18/ms    => ~% faster
Network    EnxioSocketChannel (loopback)          Records:    115.72/ms    => ~% faster
Thread     JnanomsgInproc                         Records:    117.81/ms    => ~% faster
Network    NettyDatagramEpoll (loopback)          Records:    119.89/ms    => ~% faster (unreliable)
Network    BlockingNioDatagramSocket (loopback)   Records:    124.49/ms    => ~% faster (unreliable)
???Network    NativeNettySocketIOUring (loopback)    Records:    129.90/ms    => ~22% faster
Thread     CzmqInproc                             Records:    132.52/ms    => ~24% faster
Network    NioSocketChannel (loopback)            Records:    136.40/ms    => ~% faster
Network    ChronicleNetwork (loopback)            Records:    136.54/ms    => ~% faster (using UnsafeFastJ8SocketChannel)
Network    NativeNettySocketEpoll (loopback)      Records:    137.21/ms    => ~% faster
Network    JucxStream (SoftRoCE, PEH)             Records:    137.22/ms    => ~% faster (PeerErrorHandling, unreliable, crashes after ~5 million records)
Network    NativeSocketChannel (loopback)         Records:    138.75/ms    => ~% faster (faster than NativeNetty because no JNI?)
???Network    AsyncNettyDatagramEpoll (loopback)     Records:    144.79/ms    => ~36% faster (unreliable; using async handlers for servers)
Network    BidiDisniActive (SoftiWarp)            Records:    146.68/ms    => ~% faster
Network    BidiDisniPassive (SoftiWarp)           Records:    150.68/ms    => ~% faster
Network    AsyncNettySocketEpoll (loopback)       Records:    153.22/ms    => ~% faster (using async handlers for servers)
Network    BidiBlockingHadroNioSocket (loopback)  Records:    155.16/ms    => ~% faster
Network    BidiBlockingNioSocket (loopback)       Records:    158.88/ms    => ~% faster
Process    ChronicleQueue                         Records:    164.71/ms    => ~ times as fast
Process    ChronicleQueue (tmpfs)                 Records:    164.79/ms    => ~ times as fast
Network    DisniActive (SoftiWarp)                Records:    182.08/ms    => ~% faster
Network    DisniPassive (SoftiWarp)               Records:    186.40/ms    => ~% faster
Network    BidiJucxStream (SoftRoCE, PEH)         Records:    189.48/ms    => ~% faster (PeerErrorHandling, unreliable, crashes after ~5 million records)
Network    BidiNativeMinaSocketApr (loopback)     Records:    200.83/ms    => ~% faster
Network    BidiJucxTag (SoftRoCE, PEH)            Records:    203.85/ms    => ~% faster (PeerErrorHandling, unreliable, crashes after ~5 million records)
Network    BidiEnxioSocketChannel (loopback)      Records:    205.65/ms    => ~% faster
Network    BidiNioSocketChannel (loopback)        Records:    208.31/ms    => ~% faster
Network    BidiChronicleNetwork (loopback)        Records:    212.36/ms    => ~% faster (using UnsafeFastJ8SocketChannel)
Network*   BidiNativeSocketChannel (loopback)     Records:    212.72/ms    => ~% faster
Network    JucxTag (SoftRoCE, PEH)                Records:    214.42/ms    => ~ times as fast (PeerErrorHandling, unreliable, crashes after ~5 million records)
Network*   AeronUDP (loopback)                    Records:    216.57/ms    => ~% faster (reliable and supports multicast)
Network    BidiDisniActive (SoftRoCE)             Records:    232.49/ms    => ~ times as fast (unreliable, crashes after ~1 million records)
Network    DisniActive (SoftRoCE)                 Records:    243.85/ms    => ~ times as fast (unreliable, crashes after ~1 million records)
Network    BidiDisniPassive (SoftRoCE)            Records:    245.15/ms    => ~ times as fast (unreliable, crashes after ~1 million records)
Network    NativeNettyDatagramEpoll (loopback)    Records:    263.63/ms    => ~ times as fast (unreliable)
Network    NioDatagramChannel (loopback)          Records:    286.81/ms    => ~ times as fast (unreliable)
Thread     BidiMinaVmPipe                         Records:    298.17/ms    => ~ times as fast
Network    NativeDatagramChannel (loopback)       Records:    308.10/ms    => ~ times as fast (unreliable; faster than NativeNetty because no JNI?)
Network    MinaNativeDatagramChannel (loopback)   Records:    316.37/ms    => ~% faster (unreliable)
Network    DisniPassive (SoftRoCE)                Records:    320.18/ms    => ~ times as fast (unreliable, crashes after ~1 million records)
Thread     JeromqInproc                           Records:    323.88/ms    => ~ times as fast
Process    NamedPipe (Streaming)                  Records:    351.87/ms    => ~ times as fast
Thread     MinaVmPipe                             Records:    372.10/ms    => ~ times as fast
Process    UnixDomainSocket                       Records:    429.44/ms    => ~ times as fast (requires Java 16 for NIO support)
Process    NativeUnixDomainSocket                 Records:    433.28/ms    => ~ times as fast (requires Java 16 for NIO support)
Process    BidiUnixDomainSocket                   Records:    451.21/ms    => ~ times as fast (requires Java 16 for NIO support)
Process    BidiNativeUnixDomainSocket             Records:    460.59/ms    => ~ times as fast (requires Java 16 for NIO support)
Process    NamedPipe (FileChannel)                Records:    528.44/ms    => ~ times as fast
Network    BidiJucxTag (noPEH)                    Records:    536.55/ms    => ~ times as fast (NoPeerErrorHandling, most likely using shared memory)
Network    JucxStream (noPEH)                     Records:    538.19/ms    => ~ times as fast (NoPeerErrorHandling, most likely using shared memory)
Network    JucxTag (noPEH)                        Records:    539.70/ms    => ~ times as fast (NoPeerErrorHandling, most likely using shared memory)
