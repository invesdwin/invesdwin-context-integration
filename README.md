...
Network    BidiMinaSctpApr (loopback)             Records:     27.43/ms    => ~% slower
Thread     NngInproc                              Records:     28.93/ms    => ~73% slower
Network    NettySocketIOUring (loopback)          Records:     30.49/ms    => ~% slower
Network    MinaSocketNio (loopback)               Records:     32.41/ms    => ~% slower
Network    BidiMinaSocketApr (loopback)           Records:     32.96/ms    => ~69% slower
Network    NettySocketNio (loopback)              Records:     34.74/ms    => ~% slower
Network    CzmqTcp (loopback)                     Records:     35.24/ms    => ~67% slower
Network    JnanomsgTcp (loopback)                 Records:     35.90/ms    => ~% slower
Network    AsyncMinaSctpApr (loopback)            Records:     37.99/ms    => ~% slower (using async handlers for servers)
...
Process    JeromqIpc                              Records:     41.13/ms    => ~% slower
Network    BidiNettySocketNio (loopback)          Records:     43.18/ms    => ~% slower
Network    JeromqTcp (loopback)                   Records:     43.88/ms    => ~% slower
Process    CzmqIpc                                Records:     45.13/ms    => ~58% slower
Network    FastMPJ (loopback)                     Records:     45.61/ms    => ~% slower
Network    NettyDatagramNio (loopback)            Records:     48.28/ms    => ~% slower
Network    MPJExpress (loopback)                  Records:     49.45/ms    => ~% slower
Network    AsyncMinaSocketNio (loopback)          Records:     53.45/ms    => ~% slower (using async handlers for servers)
Network    AsyncNettySocketIOUring (loopback)     Records:     54.06/ms    => ~% slower (using async handlers for servers)
Network    BidiNettySocketHadroNio (loopback)     Records:     55.29/ms    => ~% slower
Process    JzmqIpc                                Records:     55.35/ms    => ~48% slower
Network    BidiMinaDatagramNio (loopback)         Records:     55.51/ms    => ~% slower (unreliable)
Network    AsyncNettySocketOio (loopback)         Records:     58.76/ms    => ~% slower (using async handlers for servers)
Network    NettySocketHadroNio (loopback)         Records:     59.44/ms    => ~% slower
Process    JnanomsgIpc                            Records:     61.36/ms    => ~% slower
...
Network    HadroNioSocketChannel (loopback)       Records:     65.63/ms    => ~% slower
Network    AsyncNettySocketNio (loopback)         Records:     65.86/ms    => ~% faster (using async handlers for servers)
Network    BidiSctpChannel (loopback)             Records:     66.23/ms    => ~% slower
Network    BidiNativeSctpChannel (loopback)       Records:     70.97/ms    => ~% slower
Network    BidiHadroNioSocketChannel (loopback)   Records:     71.77/ms    => ~% slower
Network    AsyncMinaDatagramNio (loopback)        Records:     73.88/ms    => ~% slower (unreliable; using async handlers for servers)
Network    AsyncNettySocketHadroNio (loopback)    Records:     74.81/ms    => ~% slower (using async handlers for servers)
???Network    AsyncNettyDatagramOio (loopback)       Records:     76.74/ms    => ~28% slower (unreliable; using async handlers for servers)
Network    BlockingBidiDisniActive (SoftiWarp)    Records:     78.47/ms    => ~% slower
???Network    AsyncNettyDatagramNio (loopback)       Records:     80.97/ms    => ~24% slower (unreliable; using async handlers for servers)
Network    SctpChannel (loopback)                 Records:     81.78/ms    => ~% slower
Network    BlockingDisniActive (SoftiWarp)        Records:     84.22/ms    => ~% slower
Network    BlockingDisniActive (SoftRoCE)         Records:     84.76/ms    => ~% slower (unreliable, crashes after ~1 million records)
Network    BidiNettySocketHadroNio (SoftRoCE)     Records:     84.88/ms    => ~% slower (unreliable)
Network    NativeSctpChannel (loopback)           Records:     86.52/ms    => ~% slower
Network    BidiNativeMinaSctpApr (loopback)       Records:     87.81/ms    => ~% slower
Network    KryonetUdp (loopback)                  Records:     87.85/ms    => ~% slower
Network    BlockingBidiDisniActive (SoftRoCE)     Records:     88.53/ms    => ~% slower (unreliable, crashes after ~1 million records)
Network    NettySocketHadroNio (SoftRoCE)         Records:     95.12/ms    => ~% slower (unreliable, crashes after ~500k records)
Network    JucxTag (loopback, PEH)                Records:     97.50/ms    => ~% slower (PeerErrorHandling, supports Infiniband and other specialized hardware)
Network    BidiJucxTag (loopback, PEH)            Records:     98.78/ms    => ~% slower (PeerErrorHandling)
???Network    NettySocketEpoll (loopback)            Records:     99.24/ms    => ~7% slower
Network    JucxStream (loopback, PEH)             Records:     99.88/ms    => ~% slower (PeerErrorHandling)
Network    BidiJucxStream (loopback, PEH)         Records:    101.25/ms    => ~% slower (PeerErrorHandling)
Network    BidiHadroNioSocketChannel (SoftRoCE)   Records:    104.23/ms    => ~% slower (unreliable)
Network    HadroNioSocketChannel (SoftRoCE)       Records:    105.47/ms    => ~% slower (unreliable)
